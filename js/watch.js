...
function loadVideoAndRender(videoId, retries) {
  ...
  (async function() {
    ...
    if (retries < 5) {
      renderLoadingView();
      setTimeout(function() { loadVideoAndRender(videoId, retries + 1); }, 1000);
    } else {
      fetchFromProxyAndRetry(videoId);
    }
  })();
}

function fetchFromProxyAndRetry(videoId) {
  renderLoadingView();
  fetch('/api/supabase/videos?select=*&order=created_at.desc', {
    ...
    var found = enriched.find(function(v) { return v.id === videoId; });
    if (found) { renderPage(found); } else { renderErrorView('The video you are looking for does not exist.'); }
  }).catch(function() {
